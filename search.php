...
// Kategorileri getir - ana ve alt kategorileri hiyerarşik olarak (index.php ile aynı yapıda)
$categories = $db->query("
...
<head>
    ...
    <title><?php echo __('search_results'); ?>: <?php echo htmlspecialchars($search); ?> - <?php echo getSetting('site_title'); ?></title>
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="w-full lg:w-2/3">
                <div class="mb-6">
                    <h1 class="text-2xl font-bold text-gray-900">
                        "<?php echo htmlspecialchars($search); ?>" <?php echo __('search_results_for'); ?>
                    </h1>
                    <p class="text-gray-600 mt-2">
                        <?php echo count($articles); ?> <?php echo __('search_results_count'); ?>
                    </p>
                </div>

                <?php if (empty($articles)): ?>
                    <div class="bg-white rounded-lg shadow-lg p-6">
                        <p class="text-gray-600"><?php echo __('search_no_results'); ?></p>
                    </div>
                <?php else: ?>
                    <div class="space-y-6">
                        <?php foreach ($articles as $article): ?>
                        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                            <div class="p-6">                                <h2 class="text-xl font-semibold mb-2">
                                ...
                                <p class="text-gray-600 mb-4">
                                    <?php echo mb_substr(strip_tags(html_entity_decode($article['content'], ENT_QUOTES, 'UTF-8')), 0, 200) . '...'; ?>
                                </p>
                                <div class="flex items-center justify-between text-sm text-gray-500">
                                    <div class="flex items-center space-x-4">
                                        <span><i class="far fa-user mr-1"></i><?php echo htmlspecialchars($article['username']); ?></span>
                                        <span><i class="far fa-folder mr-1"></i><?php echo htmlspecialchars($article['category_name']); ?></span>
                                    </div>
                                    <div class="flex items-center space-x-4">
                                        <span><i class="far fa-comment mr-1"></i><?php echo $article['comment_count']; ?></span>
                                        <span><i class="far fa-clock mr-1"></i><?php echo date('d.m.Y', strtotime($article['created_at'])); ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="w-full lg:w-1/3 space-y-6">
                ...
                <!-- Arama -->
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <h3 class="text-lg font-semibold mb-4"><?php echo __('sidebar_search'); ?></h3>                    <form action="/search.php" method="get" class="flex">                        <input type="text" name="q" value="<?php echo htmlspecialchars($search); ?>" 
                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <div class="collapsible-sidebar-header" data-sidebar-id="categories">
                        <h3 class="text-lg font-semibold sidebar-heading"><?php echo __('sidebar_categories'); ?></h3>
